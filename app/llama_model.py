...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.gate_proj = nn.Linear(config["dim"], int(config["ffn_dim_multiplier"] * config["dim"]), bias=False)
        self.up_proj = nn.Linear(config["dim"], int(config["ffn_dim_multiplier"] * config["dim"]), bias=False)
        self.down_proj = nn.Linear(int(config["ffn_dim_multiplier"] * config["dim"]), config["dim"], bias=False)
...
class LlamaAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, position_ids=None):
        ...
        query_states = self.q_proj(hidden_states).view(bsz, q_len, self.n_heads, self.head_dim).transpose(1, 2)
        key_states = self.k_proj(hidden_states).view(bsz, q_len, self.n_kv_heads, self.head_dim).transpose(1, 2)
        value_states = self.v_proj(hidden_states).view(bsz, q_len, self.n_kv_heads, self.head_dim).transpose(1, 2)
